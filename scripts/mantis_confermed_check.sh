...
source ~/home/as/Документы/scr/secure_scripts/data.cfg
...
start_man_conf_task=true
  ...
  do
    ...
    do
      tasks_id=$(mysql -h $mysmantis -e "select mbt.id as 'ID' from mantis_bug_table mbt INNER JOIN mantis_category_table mct ON (mbt.category_id=mct.id) LEFT OUTER JOIN mantis_user_table mut ON (mbt.handler_id=mut.id) where mbt.status=40 and mut.username in ('a.simuskov') order by last_updated desc;")
